...
//        agoraKit.setLocalVoiceChanger(.babyGirl)
        ...
        // AudioProfile
        // MEMO: Qualityでかなりdelayが生じるっぽい
        agoraKit.setAudioProfile(.musicHighQualityStereo, scenario: .education)

        let code = agoraKit.joinChannel(byToken: nil, channelId: roomName, info: nil, uid: 0) { [unowned self] (channel, uid, elapsed) in
            print("\(channel)")
        }
        ...
        // Voice Recording
        //FIXME: 保存先を Documentsにするかは要相談だが、tmpでも良さそうではある...?
        let documentPath = NSHomeDirectory() + "/Documents"
        agoraKit.startAudioRecording(documentPath + "/voice.wav", sampleRate: 44100, quality: .high)
    }
